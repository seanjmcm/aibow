...
async function run() {
  ...
  console.log('2. Cropping...');
  const croppedBuffer = await sharp(buffer).trim().toBuffer();

  console.log('3. Applying Two Colors (Hue-based)...');
  ...
  for (let i = 0; i < data.length; i += 4) {
    const a = data[i + 3];
    if (a > 0) {
      // increase alpha contrast to reduce blurry edges
      if (a < 128) {
         data[i + 3] = 0;
         continue;
      } else {
  ...
  console.log('Successfully created perfected aibo-logo-two-colors.png');
}
