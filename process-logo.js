...
async function run() {
  console.log('Reading and cropping temp image...');
  const croppedBuffer = await sharp('aibo-logo-temp.png').trim().toBuffer();
  ...
  for (let i = 0; i < data.length; i += 4) {
    const a = data[i + 3];
    if (a > 0) {
      // Hard alpha for crisp solid edges (single color appearance with no blending)
      if (a < 128) {
         data[i + 3] = 0;
         continue;
      } else {
  ...
  .png({ quality: 100 })
  .toFile('aibo-logo-two-colors.png');

  console.log('Successfully processed to aibo-logo-two-colors.png');
}
